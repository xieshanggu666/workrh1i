...
def generate_map(seed):
    ...
    for row in range(ROWS):
        for col in range(3):
            _id = nid(row, col)
            r = row
            # 行0 普通遭遇为主；越靠后越容易出精英/奖励/锻造
            if r == 0:
                t = rng.choices([ENCOUNTER, REWARD, FORGE], [0.7, 0.2, 0.1])[0]
            elif r == 1:
                t = rng.choices([ENCOUNTER, REST, REWARD, FORGE], [0.45, 0.25, 0.2, 0.1])[0]
            elif r == 2:
                t = rng.choices([ENCOUNTER, ELITE, REWARD, FORGE], [0.35, 0.3, 0.2, 0.15])[0]
            else:
                t = rng.choices([ENCOUNTER, ELITE, REST, FORGE], [0.35, 0.4, 0.15, 0.1])[0]
